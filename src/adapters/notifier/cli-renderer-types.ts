/**
 * Types describing a generic CLI-renderer channel.
 *
 * These types are the contract between a cli-renderer (whose reference
 * implementation lives in src/runtime/daemon/cli-renderer/) and any
 * notifier backend (Telegram, Slack, terminal console, test stub) that
 * the renderer can post/edit messages on. They describe the shape of
 * the CHANNEL, not of the renderer itself, which is why they live in
 * adapters/notifier: an adapter implementing the CliRendererChannel
 * contract should not need to reach into runtime/ just to name the
 * types it implements.
 *
 * The runtime cli-renderer re-exports these types from this module so
 * consumers who think of them as "renderer types" can keep that import
 * shape.
 */
