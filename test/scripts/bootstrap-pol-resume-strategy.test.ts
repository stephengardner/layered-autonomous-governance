...
describe('bootstrap-pol-resume-strategy idempotency (smoke)', () => {
  ...
  it('first run writes the atom; second run with same shape is a no-op', async () => {
    ...
    const dir = mkdtempSync(join(tmpdir(), 'lag-bootstrap-idempotent-'));
    try {
      const host = await createFileHost({ rootDir: dir });
      const policies = buildPolicies(OP);
      const spec = policies[0]!;
  ...
  it('drift detection: a stored atom with different content surfaces a non-empty diff', async () => {
    // Smoke for the drift-detection arm of main(): if the stored
    // atom diverges from the seeded shape, diffPolicyAtom must
    // surface a non-empty list. We exercise the contract by manually
    // constructing a drifted atom and round-tripping it.
    ...
    try {
      ...
      const stored = await host.atoms.get(expected.id);
      expect(stored).not.toBeNull();
      // The stored content's enabled flag is now `false`; the
      // expected shape is `enabled: true`. A drift-detector that
      // compares these two should produce a non-empty diff.
      const storedContent = (stored!.metadata as { policy: { content: { enabled: boolean } } })
        .policy.content;
      const expectedContent = (expected.metadata as { policy: { content: { enabled: boolean } } })
        .policy.content;
      expect(storedContent.enabled).not.toBe(expectedContent.enabled);
    } finally {
      rmSync(dir, { recursive: true, force: true });
    }
  });
