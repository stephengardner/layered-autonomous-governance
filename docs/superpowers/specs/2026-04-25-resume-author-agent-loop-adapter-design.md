...
- `walkAuthorSessions(host, startingObservationAtomId)` is the example-level 2-arg helper PrFixActor's runner uses to construct its `assembleCandidates` callback (per §6.1 Option I). Staleness filtering is delegated to the strategies (each strategy enforces its own `maxStaleHours`). Tests verify the helper returns the candidate list correctly across cycle, missing-extra, and non-`pr-fix-observation` atoms in the chain, and enforces PR-boundary scoping. The helper is NOT part of the wrapper's public API; it lives in `examples/agent-loops/resume-author/walk-author-sessions.ts` as a reference for actor authors.
...
- `ResumeAuthorAgentLoopAdapter.run`: success path (strategy resolves, resume invocation returns `completed`); non-completed-result delegation to fallback; throw-from-resume delegation to fallback; no-strategy-resolves delegation to fallback. On success, the wrapper patches the underlying adapter's session atom with `extra.resumed_from_atom_id` + `extra.resume_strategy_used` (audit correlation). On fallback paths, the underlying adapter writes its own session atoms naturally; correlation is via `correlation_id` + timestamps on the failure pair.
