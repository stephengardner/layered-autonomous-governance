/**
 * Agentic brainstorm-stage adapter.
 *
 * Sibling of the single-shot reference adapter at index.ts. Runs the
 * brainstorm stage as a dispatched agent-loop session bundled with the
 * `superpowers:brainstorming` skill markdown so the agent operates
 * under the skill's discipline instead of producing a single-shot
 * paraphrase. The same `brainstormPayloadSchema` zod (re-exported from
 ...
 * - tool policy: per-principal pol-llm-tool-policy-brainstorm-actor
 *   (Read+Grep+Glob, no writes) loaded automatically by the helper.
 * - workspace: caller supplies the workspace provider; brainstorm is
...
/** Default principal for the brainstorm stage. */
const BRAINSTORM_PRINCIPAL: PrincipalId = 'brainstorm-actor' as PrincipalId;

export interface AgenticBrainstormStageConfig {
  ...
  /** Override the brainstorm-actor principal id (rarely needed). */
...
function buildBrainstormPrompt(opts: {
  ...
  return [
    ...
    '## Your turn',
    '',
    'Survey the design space for the operator-intent above. Use Read,',
    'Grep, and Glob to ground your survey in the actual codebase + canon.',
    'Your final-turn text content MUST be a single JSON object matching',
    'the BrainstormPayload schema. No prose outside the JSON.',
  ].join('\n');
}

function buildBrainstormCanonAuditPrompt(opts: {
  readonly producedOutput: BrainstormPayload;
  readonly stageInput: StageInput<unknown>;
  readonly canonAtomIds: ReadonlyArray<string>;
}): string {
  return [
    'You are a canon-compliance auditor for a deep planning pipeline.',
    'You audit the brainstorm-stage output below against the operator-',
    'intent and the applicable canon directives.',
    '',
    'Verdict criteria:',
    '- approved: output is semantically faithful to the operator-intent;',
    '  alternatives are substantively distinct; no fabricated atom-id',
    '  citations; trade-offs cite trade-offs.',
...
export function buildAgenticBrainstormStage(
  config: AgenticBrainstormStageConfig,
): PlanningStage<unknown, BrainstormPayload> {
  const stagePrincipal = config.principal ?? BRAINSTORM_PRINCIPAL;

  async function run(
    input: StageInput<unknown>,
  ): Promise<StageOutput<BrainstormPayload>> {
    const skillBundle = await resolveSkillBundle('brainstorming');
    ...
    const result = await runStageAgentLoop<BrainstormPayload>({
      stageInput: input,
      ...
      ...(config.disableCanonAudit
        ...
        : {
            canonAuditPromptBuilder: ({
              ...
              producedOutput: BrainstormPayload;
              stageInput: StageInput<unknown>;
              canonAtomIds: ReadonlyArray<string>;
            }) =>
              buildBrainstormCanonAuditPrompt({
                producedOutput,
                stageInput,
                canonAtomIds: canonAtomIds.map(String),
              }),
  ...
  return {
    ...
    // checks (cite-set membership + resolvable atom-id) apply uniformly
    // across single-shot and agentic paths. The agentic adapter's
    // canon-audit checkpoint above is supplemental, not a replacement;
    // the substrate-mediated audit() that the runner invokes after run()
    // is still the load-bearing gate.
