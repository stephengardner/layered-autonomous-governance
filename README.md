...
- **Question → Plan → PR autonomous loop.** The `cto-actor` Principal runs a PlanningActor with an LLM-backed judgment that deliberates against L3 canon and produces a Plan atom with citation-rich provenance. A `code-author` Actor consumes the Plan (after operator approval for state-mutating actors, per `pol-plan-auto-approve-low-stakes`), drafts a diff via Claude Opus with the Question's verbatim payload propagated through `metadata.question_prompt`, and opens a signed PR under a per-role GitHub App identity. `PrLandingActor` observes review state and merges when CI + CodeRabbit approval + required status all land. Complete walk-through with atom shapes, debugging checklist, and current instance-level gaps: [`docs/autonomous-org-walkthrough.md`](docs/autonomous-org-walkthrough.md).
