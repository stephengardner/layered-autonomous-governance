#!/usr/bin/env node
/**
 * Canon bootstrap for the LoopRunner stale-plan reaper TTLs.
 *
 * Seeds one L3 policy atom into the .lag/atoms store:
 *
 *   pol-reaper-ttls-default   L3 directive: warn_ms=24h abandon_ms=72h
 *
 * Promotes the env-var + CLI-flag knobs that PR #298 wired into the
 * LoopRunner from process configuration to canon configuration per
 * `dev-substrate-not-prescription`: tunable thresholds belong in atoms,
 * not constants. Existing CLI flags (--reaper-warn-ms / --reaper-abandon-ms)
 * and env vars (LAG_REAPER_WARN_MS / LAG_REAPER_ABANDON_MS via the CLI
 * fallback chain) keep working as-is; the canon path is an additive
 * higher-priority layer above them.
 *
 * Atom data lives in scripts/lib/reaper-canon-policies.mjs so the
 * test suite can drive the same builder. This wrapper handles
...
import { buildPolicies, policyAtom } from './lib/reaper-canon-policies.mjs';
...
// the script. Require explicit configuration.
const OPERATOR_ID = process.env.LAG_OPERATOR_ID;
if (!OPERATOR_ID || OPERATOR_ID.length === 0) {
  console.error(
    '[bootstrap-reaper-canon] ERROR: LAG_OPERATOR_ID is not set. Export your\n'
...
/**
 * Compare a stored reaper-policy atom's payload to the expected shape.
 * Returns a list of drift descriptors (empty = in sync). Mirrors
 * diffPolicyAtom from bootstrap-inbox-canon.mjs so the two bootstraps
 * use one drift-check pattern.
 *
...
async function main() {
  ...
  if (DRY_RUN) {
    console.log(`[bootstrap-reaper-canon] dry-run: ${policies.length} atoms would be written:`);
  ...
  for (const spec of policies) {
    ...
    if (existing === null) {
      ...
      written += 1;
      console.log(`[bootstrap-reaper-canon] wrote ${expected.id}`);
      continue;
    }
    const diffs = diffPolicyAtom(existing, expected);
    if (diffs.length > 0) {
      console.error(
        `[bootstrap-reaper-canon] DRIFT on ${expected.id}:\n  ${diffs.join('\n  ')}\n`
          + 'Resolve by: (a) editing buildPolicies in scripts/lib/reaper-canon-policies.mjs '
  ...
  console.log(`[bootstrap-reaper-canon] done. ${written} written, ${ok} already in sync.`);
}

main().catch((err) => {
  console.error('[bootstrap-reaper-canon] FAILED:', err);
  process.exit(1);
});
