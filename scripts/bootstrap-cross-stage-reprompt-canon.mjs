#!/usr/bin/env node
/**
 * Canon bootstrap for the planning-pipeline cross-stage re-prompt gate.
 *
 * Seeds one L3 policy atom into the .lag/atoms store:
 *
 *   pol-cross-stage-reprompt-default  L3 directive: max_attempts=2,
 *                                     severities_to_reprompt=['critical'],
 *                                     allowed_targets='derive-from-pipeline-composition'
 *
 * Activates the runner's cross-stage re-prompt path: when a stage's
 * audit() returns a finding whose reprompt_target cites an upstream
 * stage AND severity is critical, the runner walks back to the target
 * stage and re-invokes the chain with the finding folded into
 * priorAuditFindings.
 *
 * Asymmetric with bootstrap-auditor-feedback-reprompt-canon: that
 * script seeds an atom that mirrors a hardcoded floor; this one
 * ACTIVATES a feature that is otherwise dormant. Running this is the
 * deliberate operator decision to turn cross-stage routing on.
 *
 * The runner consumes the policy via readCrossStageRePromptPolicy(host)
 * and short-circuits to the dormant path when no atom resolves; this
 * script seeds the atom so the gate becomes active.
 *
 * Atom data lives in
 * scripts/lib/cross-stage-reprompt-canon-policies.mjs so the test
...
import { buildPolicies, policyAtom } from './lib/cross-stage-reprompt-canon-policies.mjs';
...
if (!OPERATOR_ID) {
  console.error(
    '[bootstrap-cross-stage-reprompt-canon] ERROR: LAG_OPERATOR_ID is not set. Export your\n'
...
/**
 * Compare a stored cross-stage-reprompt policy atom's payload to the
 * expected shape. Returns a list of drift descriptors (empty = in
...
async function main() {
  ...
  if (DRY_RUN) {
    console.log(
      `[bootstrap-cross-stage-reprompt-canon] dry-run: ${policies.length} atoms would be written:`,
    );
  ...
  for (const spec of policies) {
    ...
    if (existing === null) {
      ...
      written += 1;
      console.log(`[bootstrap-cross-stage-reprompt-canon] wrote ${expected.id}`);
      continue;
    }
    const diffs = diffPolicyAtom(existing, expected);
    if (diffs.length > 0) {
      console.error(
        `[bootstrap-cross-stage-reprompt-canon] DRIFT on ${expected.id}:\n  ${diffs.join('\n  ')}\n`
          + 'Resolve by: (a) editing buildPolicies in '
          + 'scripts/lib/cross-stage-reprompt-canon-policies.mjs to match stored shape if the '
  ...
  console.log(`[bootstrap-cross-stage-reprompt-canon] done. ${written} written, ${ok} already in sync.`);
}

main().catch((err) => {
  console.error('[bootstrap-cross-stage-reprompt-canon] FAILED:', err);
  process.exit(1);
});
