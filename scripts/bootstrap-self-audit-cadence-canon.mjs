#!/usr/bin/env node
/**
 * Canon bootstrap for the self-audit cadence policy.
 *
 * Seeds one L3 policy atom into the .lag/atoms store:
 *
 *   pol-self-audit-cadence   L3 directive: enabled=false, intervalMs=3600000
 *
 * Promotes the LoopRunner.runSelfAuditPass option (CLI / env) to canon
 * configuration per `dev-substrate-not-prescription`: a tunable
 * cadence + enable knob belongs in atoms, not constants. The driver
 * script and LoopRunner pass keep working as-is; the canon path is an
 * additive higher-priority layer above them.
 *
 * Atom data lives in scripts/lib/self-audit-cadence-canon-policies.mjs
 ...
 * (same discipline as bootstrap-loop-pass-claim-reaper-canon.mjs).
 */
...
import { buildPolicies, policyAtom } from './lib/self-audit-cadence-canon-policies.mjs';
...
if (!OPERATOR_ID) {
  console.error(
    '[bootstrap-self-audit-cadence-canon] ERROR: LAG_OPERATOR_ID is not set. Export your\n'
...
/**
 * Compare a stored self-audit-cadence policy atom's payload to the
 * expected shape. Returns a list of drift descriptors (empty = in
 * sync). Mirrors diffPolicyAtom from bootstrap-loop-pass-claim-reaper-canon.mjs
...
async function main() {
  ...
  if (DRY_RUN) {
    console.log(
      `[bootstrap-self-audit-cadence-canon] dry-run: ${policies.length} atoms would be written:`,
    );
  ...
  for (const spec of policies) {
    ...
    if (existing === null) {
      ...
      written += 1;
      console.log(`[bootstrap-self-audit-cadence-canon] wrote ${expected.id}`);
      continue;
    }
    const diffs = diffPolicyAtom(existing, expected);
    if (diffs.length > 0) {
      console.error(
        `[bootstrap-self-audit-cadence-canon] DRIFT on ${expected.id}:\n  ${diffs.join('\n  ')}\n`
          + 'Resolve by: (a) editing buildPolicies in '
          + 'scripts/lib/self-audit-cadence-canon-policies.mjs to match stored shape if the '
  ...
  console.log(`[bootstrap-self-audit-cadence-canon] done. ${written} written, ${ok} already in sync.`);
}

main().catch((err) => {
  console.error('[bootstrap-self-audit-cadence-canon] FAILED:', err);
  process.exit(1);
});
